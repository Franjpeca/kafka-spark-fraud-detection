## Desarrollo de Kafka y microservicios

- [X] Configurar contenedores Docker (Kafka, Zookeeper, Mongo, etc.)
- [ ] Crear un productor simple que lea datos de un archivo Parquet y los envíe a Kafka
- [ ] Crear un consumidor básico que lea mensajes de Kafka y los imprima
- [ ] Desarrollar el generador de eventos realista (para simular datos más realistas)
- [ ] Reemplazar el consumidor básico por un consumidor de streaming con procesamiento real
- [ ] Integrar más consumidores y microservicios especializados (almacenamiento, alertas, cálculos)
- [ ] Realizar optimización del sistema y pruebas de rendimiento
- [ ] Implementar validación de esquemas y monitoreo (con herramientas como ELK o Prometheus)
